...
with col_collect:
    if st.button("📦 收集数据", use_container_width=True, help="收集当前筛选的壳体数据用于分析"):
        st.session_state.show_test_analysis_collection_dialog = True
...
if filtered_df.empty:
    store_test_analysis_filtered(None)
    st.warning("筛选条件下没有数据，请调整过滤器。")
    st.stop()

store_test_analysis_filtered(filtered_df.copy())

# 数据收集对话框
if st.session_state.get('show_test_analysis_collection_dialog', False):
    # 获取当前筛选的壳体号
    if "壳体号" in filtered_df.columns:
        shell_ids = sorted(filtered_df["壳体号"].unique().tolist())
        ...
        if shell_ids:
            ...
            if confirm:
                ...
                with st.spinner('正在收集数据...'):
                    ...
                    if success:
                        st.success(message)
                        st.session_state.show_test_analysis_collection_dialog = False
                    else:
                        st.error(message)
        else:
