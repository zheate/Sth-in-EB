...
with col_collect:
    if st.button("📦 收集数据", use_container_width=True, help="收集当前显示的壳体数据用于分析"):
        st.session_state.show_progress_collection_dialog = True
...
if st.session_state.progress_df is not None:
    ...
    if len(progress_df) > 0:
        ...
        if filtered_progress_df.empty:
            store_progress_filtered(None)
            st.warning("筛选条件下没有数据，请调整生产订单选择。")
        else:
            store_progress_filtered(filtered_progress_df.copy())
            # 数据收集对话框
            if st.session_state.get('show_progress_collection_dialog', False):
                # 获取当前显示的壳体号
                if "壳体号" in filtered_progress_df.columns:
                    shell_ids = sorted(filtered_progress_df["壳体号"].unique().tolist())
                        ...
                        if confirm:
                            ...
                            with st.spinner('正在收集数据...'):
                                ...
                                if data_sources.get('data_fetch', True):
                                    shared_fetch_df = get_data_fetch_result()
                                    if shared_fetch_df is not None and not shared_fetch_df.empty:
                                        data_fetch_data = DataCollector.collect_from_data_fetch(shared_fetch_df, shell_ids)
                                ...
                                if success:
                                    st.success(message)
                                    st.session_state.show_progress_collection_dialog = False
                                else:
                                    st.error(message)
                    else:
                        st.warning("当前筛选条件下没有可收集的壳体数据。")
                        st.session_state.show_progress_collection_dialog = False
                else:
                    st.warning("数据中没有壳体号列。")
                    st.session_state.show_progress_collection_dialog = False
