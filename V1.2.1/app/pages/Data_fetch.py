...
def _prepare_metric_series(df: pd.DataFrame, metric_column: str) -> pd.DataFrame:
    """Clean current/metric pairs and aggregate duplicate current values."""
...
def _clean_metric_dataframe(df: pd.DataFrame, metric_column: str) -> pd.DataFrame:
    numeric = df[[CURRENT_COLUMN, metric_column]].copy()
    numeric[CURRENT_COLUMN] = pd.to_numeric(
        numeric[CURRENT_COLUMN],
        errors="coerce",
    )
    numeric[metric_column] = pd.to_numeric(
        numeric[metric_column],
        errors="coerce",
    )
    numeric = numeric.dropna(subset=[CURRENT_COLUMN, metric_column])
    numeric = numeric.sort_values(CURRENT_COLUMN)
    return numeric.rename(columns={CURRENT_COLUMN: "current", metric_column: "value"})
...
def read_excel_with_engine(file_path: Path, sheet_name: Union[int, str] = 0, **kwargs: Any) -> pd.DataFrame:
    ...
    for encoding in ("utf-8", "utf-8-sig", "gbk", "gb18030", "latin1"):
        ...
        try:
            ...
            return pd.read_csv(file_path, sep=None, engine="python", encoding=encoding, **kwargs)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("📈 单壳体分析", width='stretch'):
        ...
        if st.button("📉 多壳体分析", width='stretch'):
        ...
        if st.button("🔄 多站别分析", width='stretch'):
        ...
        if st.button("📦 箱线图分析", width='stretch'):
        ...
        if EXTRACTION_STATE_KEY in st.session_state and st.session_state[EXTRACTION_STATE_KEY]:
            ...
            if extraction_state and "result_df" in extraction_state and extraction_state["result_df"] is not None:
                ...
                with col1:
                    st.metric("壳体数", len(extraction_state.get("folder_entries", [])), label_visibility="visible")
                with col2:
                    st.metric("数据行", len(result_df), label_visibility="visible")
                with col3:
                    if TEST_TYPE_COLUMN in result_df.columns:
    ...
    with st.form("input_form"):
        folder_input = st.text_area(
            "壳体号或Ldtd路径",
            help="可输入一个或多个，每行一个，如 HHD550048。也支持直接粘贴完整路径。",
        )
        ...
        selected_measurements = st.multiselect(
            ...
            default=list(MEASUREMENT_OPTIONS.keys()),
        ...
        current_input = st.text_input(
            ...
            help="可选，默认最高电流点，适用于 LVI 与 Rth，可输入单值或范围如 12~19，多个值用逗号或换行分隔。",
    ...
    if st.session_state.get('show_boxplot', False):
        ...
        if 'lvi_plot_sources' in st.session_state and st.session_state.lvi_plot_sources:
            st.subheader("📊 箱线图分析")
            ...
            # 收集所有壳体的数据
            all_data_for_boxplot = []
            for (shell_id, test_type), (df_full, df_selected) in st.session_state.lvi_plot_sources.items():
                if df_full is None or df_full.empty:
                    continue
                if CURRENT_COLUMN not in df_full.columns:
                    continue
                ...
                base_df[TEST_TYPE_COLUMN] = test_type.replace("测试", "")
                base_df[SHELL_COLUMN] = shell_id
                all_data_for_boxplot.append(base_df)
            
            if all_data_for_boxplot:
                combined_boxplot_df = pd.concat(all_data_for_boxplot, ignore_index=True)
                
                # 合并Rth数据（波长和shift）
                if 'rth_plot_sources' in st.session_state and st.session_state.rth_plot_sources:
                    rth_data_list = []
                    for (shell_id, test_type), rth_df in st.session_state.rth_plot_sources.items():
                        if rth_df is not None and not rth_df.empty:
                            rth_temp = rth_df.copy()
                            if CURRENT_COLUMN not in rth_temp.columns:
                                continue
                            if selected_currents:
                            ...
                            # 只保留需要的列
                            rth_cols = [SHELL_COLUMN, TEST_TYPE_COLUMN, CURRENT_COLUMN]
                            if LAMBDA_COLUMN in rth_temp.columns:
                                rth_cols.append(LAMBDA_COLUMN)
                            if SHIFT_COLUMN in rth_temp.columns:
                                rth_cols.append(SHIFT_COLUMN)
                            rth_data_list.append(rth_temp[rth_cols])
                    
                    if rth_data_list:
                        rth_combined = pd.concat(rth_data_list, ignore_index=True)
                        # 使用outer join合并，如果已有波长列则更新
                        if LAMBDA_COLUMN in combined_boxplot_df.columns or SHIFT_COLUMN in combined_boxplot_df.columns:
                            # 先删除已有的波长列（如果存在）
                            cols_to_drop = []
                            if LAMBDA_COLUMN in combined_boxplot_df.columns:
                                cols_to_drop.append(LAMBDA_COLUMN)
                            if SHIFT_COLUMN in combined_boxplot_df.columns:
                                cols_to_drop.append(SHIFT_COLUMN)
                            if cols_to_drop:
                ...
                try:
                    ...
                    with boxplot_tabs[tab_idx]:
                        tab_idx += 1
                        power_boxplot_data = combined_boxplot_df[[TEST_TYPE_COLUMN, POWER_COLUMN]].dropna()
                        
                        if not power_boxplot_data.empty:
                            # 检查每个站别的数据点数量和数据变化
                            station_counts = power_boxplot_data.groupby(TEST_TYPE_COLUMN).size()
                            stations_with_enough_data = station_counts[station_counts >= 2].index.tolist()
                            
                            # 检查数据是否有变化（标准差>阈值，避免浮点数精度问题）
                            stations_with_data = []
                            stations_no_variance = []
                            variance_threshold = 1e-10  # 设置一个合理的阈值
                            for station in stations_with_enough_data:
                                station_data = power_boxplot_data[power_boxplot_data[TEST_TYPE_COLUMN] == station][POWER_COLUMN]
                                if station_data.std() > variance_threshold:
                            ...
                            if stations_with_data:
                                # 只显示有足够数据点的站别
                                power_boxplot_filtered = power_boxplot_data[power_boxplot_data[TEST_TYPE_COLUMN].isin(stations_with_data)]
                                # 按站别排序
                                power_boxplot_filtered['__order'] = power_boxplot_filtered[TEST_TYPE_COLUMN].map(SANITIZED_ORDER_LOOKUP)
                                power_boxplot_filtered = power_boxplot_filtered.sort_values('__order').drop(columns=['__order'])
                                ...
                                chart = alt.Chart(power_boxplot_filtered).mark_boxplot(extent='min-max', size=50).encode(
                                    x=alt.X(f"{TEST_TYPE_COLUMN}:N", title="Station", sort=present_stations, 
                                           axis=alt.Axis(labelAngle=-45)),
                                    y=alt.Y(f"{POWER_COLUMN}:Q", title="功率(W)", scale=alt.Scale(zero=False)),
                                ...
                                if stations_insufficient:
                                    warnings.append(f"数据点不足（需要至少2个壳体）：{', '.join(stations_insufficient)}")
                                if stations_no_variance:
                                    warnings.append(f"数据无变化（所有值相同）：{', '.join(stations_no_variance)}")
                                if warnings:
                                    st.caption("⚠️ " + "；".join(warnings))
                            else:
                                if stations_no_variance:
                                    st.info(f"以下站别数据无变化（所有壳体的功率值相同）：{', '.join(stations_no_variance)}")
                                else:
                                    st.info("各站别数据点不足（箱线图需要至少2个壳体的数据）")
                        else:
                            st.info("无功率数据")
                    ...
                    with boxplot_tabs[tab_idx]:
                        tab_idx += 1
                        eff_boxplot_data = combined_boxplot_df[[TEST_TYPE_COLUMN, EFFICIENCY_COLUMN]].copy()
                        eff_boxplot_data[EFFICIENCY_COLUMN] = pd.to_numeric(eff_boxplot_data[EFFICIENCY_COLUMN], errors='coerce') * 100
                        eff_boxplot_data = eff_boxplot_data.dropna()
                        if not eff_boxplot_data.empty:
                            # 检查每个站别的数据点数量和数据变化
                            station_counts = eff_boxplot_data.groupby(TEST_TYPE_COLUMN).size()
                            stations_with_enough_data = station_counts[station_counts >= 2].index.tolist()
                            ...
                            variance_threshold = 1e-10  # 设置一个合理的阈值
                            for station in stations_with_enough_data:
                                station_data = eff_boxplot_data[eff_boxplot_data[TEST_TYPE_COLUMN] == station][EFFICIENCY_COLUMN]
                                if station_data.std() > variance_threshold:
                                    stations_with_data.append(station)
                                else:
                                    stations_no_variance.append(station)
                            ...
                            if stations_with_data:
                                # 只显示有足够数据点的站别
                                eff_boxplot_filtered = eff_boxplot_data[eff_boxplot_data[TEST_TYPE_COLUMN].isin(stations_with_data)]
                                # 按站别排序
                                eff_boxplot_filtered['__order'] = eff_boxplot_filtered[TEST_TYPE_COLUMN].map(SANITIZED_ORDER_LOOKUP)
                                eff_boxplot_filtered = eff_boxplot_filtered.sort_values('__order').drop(columns=['__order'])
                                
                                # 获取实际存在的站别及其颜色
                                present_stations = [s for s in SANITIZED_PLOT_ORDER if s in stations_with_data]
                                present_colors = [STATION_COLORS.get(s, "#000084") for s in present_stations]
                                
                                chart = alt.Chart(eff_boxplot_filtered).mark_boxplot(extent='min-max', size=50).encode(
                                    x=alt.X(f"{TEST_TYPE_COLUMN}:N", title="Station", sort=present_stations,
                                           axis=alt.Axis(labelAngle=-45)),
                                    y=alt.Y(f"{EFFICIENCY_COLUMN}:Q", title="效率(%)", scale=alt.Scale(zero=False)),
                                    color=alt.Color(f"{TEST_TYPE_COLUMN}:N", legend=None,
                                                   scale=alt.Scale(domain=present_stations, range=present_colors)),
                                    tooltip=[
                                        alt.Tooltip(f"{TEST_TYPE_COLUMN}:N", title="Station"),
                                ...
                                if stations_no_variance:
                                    st.info(f"以下站别数据无变化（所有壳体的效率值相同）：{', '.join(stations_no_variance)}")
                                else:
                                    st.info("各站别数据点不足（箱线图需要至少2个壳体的数据）")
                        else:
                            st.info("无效率数据")
                    ...
                    with boxplot_tabs[tab_idx]:
                        tab_idx += 1
                        voltage_boxplot_data = combined_boxplot_df[[TEST_TYPE_COLUMN, VOLTAGE_COLUMN]].dropna()
                        
                        if not voltage_boxplot_data.empty:
                            # 检查每个站别的数据点数量和数据变化
                            station_counts = voltage_boxplot_data.groupby(TEST_TYPE_COLUMN).size()
                            stations_with_enough_data = station_counts[station_counts >= 2].index.tolist()
                            ...
                            variance_threshold = 1e-10  # 设置一个合理的阈值
                            for station in stations_with_enough_data:
                                station_data = voltage_boxplot_data[voltage_boxplot_data[TEST_TYPE_COLUMN] == station][VOLTAGE_COLUMN]
                            ...
                            if stations_with_data:
                                # 只显示有足够数据点的站别
                                voltage_boxplot_filtered = voltage_boxplot_data[voltage_boxplot_data[TEST_TYPE_COLUMN].isin(stations_with_data)]
                                # 按站别排序
                                voltage_boxplot_filtered['__order'] = voltage_boxplot_filtered[TEST_TYPE_COLUMN].map(SANITIZED_ORDER_LOOKUP)
                                voltage_boxplot_filtered = voltage_boxplot_filtered.sort_values('__order').drop(columns=['__order'])
                                
                                # 获取实际存在的站别及其颜色
                                present_stations = [s for s in SANITIZED_PLOT_ORDER if s in stations_with_data]
                                present_colors = [STATION_COLORS.get(s, "#000084") for s in present_stations]
                                
                                chart = alt.Chart(voltage_boxplot_filtered).mark_boxplot(extent='min-max', size=50).encode(
                                    x=alt.X(f"{TEST_TYPE_COLUMN}:N", title="Station", sort=present_stations,
                                           axis=alt.Axis(labelAngle=-45)),
                                    y=alt.Y(f"{VOLTAGE_COLUMN}:Q", title="电压(V)", scale=alt.Scale(zero=False)),
                                    color=alt.Color(f"{TEST_TYPE_COLUMN}:N", legend=None,
                                                   scale=alt.Scale(domain=present_stations, range=present_colors)),
                                    tooltip=[
                                        alt.Tooltip(f"{TEST_TYPE_COLUMN}:N", title="Station"),
                                ...
                                if stations_no_variance:
                                    st.info(f"以下站别数据无变化（所有壳体的电压值相同）：{', '.join(stations_no_variance)}")
                                else:
                                    st.info("各站别数据点不足（箱线图需要至少2个壳体的数据）")
                        else:
                            st.info("无电压数据")
                    ...
                    if has_lambda:
                        with boxplot_tabs[tab_idx]:
                            tab_idx += 1
                            lambda_boxplot_data = combined_boxplot_df[[TEST_TYPE_COLUMN, LAMBDA_COLUMN]].dropna()
                            if not lambda_boxplot_data.empty:
                                # 检查每个站别的数据点数量和数据变化
                                station_counts = lambda_boxplot_data.groupby(TEST_TYPE_COLUMN).size()
                                stations_with_enough_data = station_counts[station_counts >= 2].index.tolist()
                                ...
                                variance_threshold = 1e-10  # 设置一个合理的阈值
                                for station in stations_with_enough_data:
                                    station_data = lambda_boxplot_data[lambda_boxplot_data[TEST_TYPE_COLUMN] == station][LAMBDA_COLUMN]
                                    if station_data.std() > variance_threshold:
                                        stations_with_data.append(station)
                                    else:
                                        stations_no_variance.append(station)
                                ...
                                if stations_with_data:
                                    # 只显示有足够数据点的站别
                                    lambda_boxplot_filtered = lambda_boxplot_data[lambda_boxplot_data[TEST_TYPE_COLUMN].isin(stations_with_data)]
                                    # 按站别排序
                                    lambda_boxplot_filtered['__order'] = lambda_boxplot_filtered[TEST_TYPE_COLUMN].map(SANITIZED_ORDER_LOOKUP)
                                    lambda_boxplot_filtered = lambda_boxplot_filtered.sort_values('__order').drop(columns=['__order'])
                                    
                                    # 获取实际存在的站别及其颜色
                                    present_stations = [s for s in SANITIZED_PLOT_ORDER if s in stations_with_data]
                                    present_colors = [STATION_COLORS.get(s, "#000084") for s in present_stations]
                                    
                                    chart = alt.Chart(lambda_boxplot_filtered).mark_boxplot(extent='min-max', size=50).encode(
                                        x=alt.X(f"{TEST_TYPE_COLUMN}:N", title="Station", sort=present_stations,
                                               axis=alt.Axis(labelAngle=-45)),
                                        y=alt.Y(f"{LAMBDA_COLUMN}:Q", title="波长(nm)", scale=alt.Scale(zero=False)),
                                        color=alt.Color(f"{TEST_TYPE_COLUMN}:N", legend=None,
                                                       scale=alt.Scale(domain=present_stations, range=present_colors)),
                                    tooltip=[
                                        alt.Tooltip(f"{TEST_TYPE_COLUMN}:N", title="Station"),
                                    ...
                                    if stations_no_variance:
                                        st.info(f"以下站别数据无变化（所有壳体的波长值相同）：{', '.join(stations_no_variance)}")
                                    else:
                                        st.info("各站别数据点不足（箱线图需要至少2个壳体的数据）")
                            else:
                                st.info("无波长数据")
                    ...
                    if has_shift:
                        with boxplot_tabs[tab_idx]:
                            shift_boxplot_data = combined_boxplot_df[[TEST_TYPE_COLUMN, SHIFT_COLUMN]].dropna()
                            if not shift_boxplot_data.empty:
                                # 检查每个站别的数据点数量和数据变化
                                station_counts = shift_boxplot_data.groupby(TEST_TYPE_COLUMN).size()
                                stations_with_enough_data = station_counts[station_counts >= 2].index.tolist()
                                ...
                                variance_threshold = 1e-10  # 设置一个合理的阈值
                                for station in stations_with_enough_data:
                                    station_data = shift_boxplot_data[shift_boxplot_data[TEST_TYPE_COLUMN] == station][SHIFT_COLUMN]
                                    if station_data.std() > variance_threshold:
                                        stations_with_data.append(station)
                                    else:
                                        stations_no_variance.append(station)
                                ...
                                if stations_with_data:
                                    # 只显示有足够数据点的站别
                                    shift_boxplot_filtered = shift_boxplot_data[shift_boxplot_data[TEST_TYPE_COLUMN].isin(stations_with_data)]
                                    # 按站别排序
                                    shift_boxplot_filtered['__order'] = shift_boxplot_filtered[TEST_TYPE_COLUMN].map(SANITIZED_ORDER_LOOKUP)
                                    shift_boxplot_filtered = shift_boxplot_filtered.sort_values('__order').drop(columns=['__order'])
                                    
                                    # 获取实际存在的站别及其颜色
                                    present_stations = [s for s in SANITIZED_PLOT_ORDER if s in stations_with_data]
                                    present_colors = [STATION_COLORS.get(s, "#000084") for s in present_stations]
                                    
                                    chart = alt.Chart(shift_boxplot_filtered).mark_boxplot(extent='min-max', size=50).encode(
                                        x=alt.X(f"{TEST_TYPE_COLUMN}:N", title="Station", sort=present_stations,
                                               axis=alt.Axis(labelAngle=-45)),
                                        y=alt.Y(f"{SHIFT_COLUMN}:Q", title="波长Shift(nm)", scale=alt.Scale(zero=False)),
                                        color=alt.Color(f"{TEST_TYPE_COLUMN}:N", legend=None,
                                                       scale=alt.Scale(domain=present_stations, range=present_colors)),
                                    tooltip=[
                                        alt.Tooltip(f"{TEST_TYPE_COLUMN}:N", title="Station"),
                                    ...
                                    if stations_no_variance:
                                        st.info(f"以下站别数据无变化（所有壳体的波长Shift值相同）：{', '.join(stations_no_variance)}")
                                    else:
                                        st.info("各站别数据点不足（箱线图需要至少2个壳体的数据）")
                            else:
                                st.info("无波长Shift数据")
                    ...
                    st.error("需要安装 Altair：pip install altair")
            else:
                st.info("无可用数据")
    ...
    if st.session_state.get('show_multi_station', False):
        if 'lvi_plot_sources' in st.session_state and st.session_state.lvi_plot_sources:
            ...
            if len(available_shells) > 1:
                ...
                # 收集所有壳体的数据，按站别分组计算平均值
                all_shells_data = []
                for shell_id in available_shells:
                    for (sid, test_type), (df_full, df_selected) in st.session_state.lvi_plot_sources.items():
                    ...
                    # 添加Rth数据
                    if 'rth_plot_sources' in st.session_state and st.session_state.rth_plot_sources:
                        for (sid, test_type), rth_df in st.session_state.rth_plot_sources.items():
                            if sid == shell_id and rth_df is not None and not rth_df.empty:
                                for i, lvi_df in enumerate(all_shells_data):
                                    if lvi_df[SHELL_COLUMN].iloc[0] == shell_id and lvi_df[TEST_TYPE_COLUMN].iloc[0] == test_type.replace("测试", ""):
                                        rth_temp = rth_df.copy()
                                        rth_temp[TEST_TYPE_COLUMN] = test_type.replace("测试", "")
                                        rth_temp[SHELL_COLUMN] = shell_id
                                        merged = pd.merge(
                ...
                if all_shells_data:
                    ...
                    # 获取有序的站别列表
                    test_types_ordered_avg = [t for t in SANITIZED_PLOT_ORDER if t in avg_by_station[TEST_TYPE_COLUMN].unique()]
                    
                    # 计算站别间变化
                    avg_change_data: List[Dict[str, Union[str, float]]] = []
                    for i in range(len(test_types_ordered_avg) - 1):
                        from_type = test_types_ordered_avg[i]
                        to_type = test_types_ordered_avg[i + 1]
                    ...
                    if avg_change_data:
                        ...
                        avg_numeric_cols = [col for col in avg_change_df.columns if col != '变化']

                        # 将数值四舍五入到3位小数，并将接近0的值设为精确的0
                        for col in avg_numeric_cols:
                            if col in avg_change_df.columns:
            ...
            if extraction_state and "result_df" in extraction_state and extraction_state["result_df"] is not None:
                ...
                available_metrics = [column for column in analysis_columns if column in result_df.columns]
                per_type_records: List[Dict[str, Any]] = []

                if available_metrics and TEST_TYPE_COLUMN in result_df.columns:
                    for test_type, group in result_df.groupby(TEST_TYPE_COLUMN):
                ...
                if available_metrics:
                    with st.expander("📊 指标分析", expanded=True):
                        if TEST_TYPE_COLUMN in result_df.columns:
                            available_test_types = [t for t in SANITIZED_PLOT_ORDER if t in result_df[TEST_TYPE_COLUMN].unique()]
                            if available_test_types:
                                ...
                                if selected_test_type == "全部":
                                    numeric_data = result_df[available_metrics].apply(pd.to_numeric, errors="coerce")
                                    st.markdown("### 📈 全部数据统计")
                                else:
                                    selected_test_df = result_df[result_df[TEST_TYPE_COLUMN] == selected_test_type]
    ...
    if not action_requested and extraction_state is None:
        ...
        st.info("填写参数后点击“开始提取”或“生成电流-功率-电光效率图”。")
    ...
    if action_requested:
        ...
        if not folder_input:
            ...
            st.toast("❌ 请填写壳体号", icon="❌")

            return

        if not selected_tests:

            st.toast("❌ 请至少选择一个测试类型", icon="❌")

            return

        if not selected_measurements:

            st.toast("❌ 请至少选择一个测试文件", icon="❌")
        ...
        if not folder_entries:
            ...
            st.toast("❌ 未识别到有效的文件夹输入，请检查格式", icon="❌")
    ...
    if action_requested and should_recompute:
        ...
        for idx, entry in enumerate(folder_entries, start=1):
            try:
                ...
                status_text.text(f"处理中: {idx}/{total_entries} - {entry} (路径错误)")
                continue
            ...
            for test_category in selected_tests:
                try:
                    ...
                    measurement_index = _build_measurement_file_index(test_folder, files_in_folder)
                ...
                for measurement_label in selected_measurements:
                    ...
                    try:
                        ...
                        # 调试信息：显示找到的文件
                        info_messages.append(f"找到文件: {entry}/{test_category}/{measurement_label} -> {file_path.name}")
                        ...
                        if measurement_label == "LVI":
                            ...
                            if missing_currents:
                                info_messages.append(
                                    f"{entry}/{test_category}/{measurement_label}: 未找到电流点 {missing_currents}"
                                )

                            selected_subset = extracted if current_points else None
                            lvi_plot_sources[(entry, test_category)] = (lvi_full, selected_subset)
                            ...
                            rth_plot_sources[(entry, test_category)] = rth_full_numeric
                            ...
                            if rth_summary_parts:
                                info_messages.append(
                                    f"{entry}/{test_category}/{measurement_label}: {', '.join(rth_summary_parts)}"
                                )
                            ...
                            if baseline_current is not None and abs(baseline_current - 2.0) > CURRENT_TOLERANCE:
                                info_messages.append(
                                    f"{entry}/{test_category}/{measurement_label}: 波长shift基准使用 {baseline_current:.3f}A（未找到 2A 数据）"
                                )
                    ...
                    tagged.insert(0, SHELL_COLUMN, entry)
                    ...
                    if multiple_found:
                        info_messages.append(
                            f"{entry}/{test_category}/{measurement_label}: 使用最新文件 {file_path.name}"
                        )
        
        # 更新进度
            progress_bar.progress(min(idx / total_entries, 1.0))
            status_text.text(f"已完成: {idx}/{total_entries} - {entry}")
