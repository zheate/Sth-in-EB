...
MATERIAL_FILE = _CURRENT_DIR / "material.json"
INPUT_FILE = _CURRENT_DIR / "BFD_Calculator_input.json"
...
def material_manager():
    ...
    with col2:
        ...
        with btn_col1:
            if st.button("💾 保存", use_container_width=True, key="bfd_save_material"):
        ...
        with btn_col2:
            if st.button("🗑️ 删除", use_container_width=True, disabled=not selected_material, key="bfd_delete_material"):
        ...
        with btn_col3:
            if st.button("❌ 关闭", use_container_width=True, key="bfd_close_material"):
                st.session_state.show_material_manager = False
                st.rerun()
...
def main():
    ...
    with col1:
        if st.button("📦 管理材料", use_container_width=True, key="bfd_manage_material"):
            st.session_state.show_material_manager = not st.session_state.show_material_manager
            st.rerun()
    
    with col2:
        if st.button("ℹ️ 公式说明", use_container_width=True, key="bfd_formula_help"):
    ...
    with settings_col2:
        ...
        if has_endcap:
            ...
            with endcap_sub_col3:
                ...
                try:
                    endcap_length_val = float(endcap_length) if endcap_length else 0
                except ValueError:
                    endcap_length_val = 0
    ...
    # 计算按钮
    if st.button("🧮 计算 BFD", type="primary", use_container_width=True, key="bfd_calculate"):
        # 验证输入
        errors = []
